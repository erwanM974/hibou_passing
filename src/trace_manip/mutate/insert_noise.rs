...
fn generate_random_action_on_coloc(rng : &mut ThreadRng, gen_ctx : &GeneralContext, coloc_lfs : &HashSet<usize>) -> TraceAction {
    ...
    if rng.gen::<bool>() {
        return TraceAction::new(lf_id,TraceActionKind::Emission,MessageTypeExpression::Singleton(ms_id));
    } else {
        return TraceAction::new(lf_id,TraceActionKind::Reception,MessageTypeExpression::Singleton(ms_id));
    }
}
