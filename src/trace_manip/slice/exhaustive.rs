    ...
    match rem_canals.next() {
        ...
        Some(canal_trace) => {
            ...
            while let Some(got_slice) = slicer.next() {
                let mut new_trace = got_slice.iter().cloned().collect::<Vec<TraceAction>>();
    ...
    match rem_canals.next() {
        ...
        Some(canal_trace ) => {
            for i in 0..(canal_trace.len()+1) {
                let mut new_trace = canal_trace[0..i].iter().cloned().collect::<Vec<TraceAction>>();
                let mut new_ok_canals = ok_canals.clone();
                new_ok_canals.push(new_trace);
...
pub fn get_all_suffixes_rec<'a>(gen_ctx : &GeneralContext,
    ...
    match rem_canals.next() {
        ...
        Some(canal_trace) => {
            for i in 0..(canal_trace.len()+1) {
                let mut new_trace = canal_trace[i..].iter().cloned().collect::<Vec<TraceAction>>();
                let mut new_ok_canals = ok_canals.clone();
                new_ok_canals.push(new_trace);
