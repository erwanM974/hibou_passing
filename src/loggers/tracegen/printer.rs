...
impl MultiTraceProcessPrinter {
    ...
    pub(crate) fn add_action_to_multi_trace(&self,
                                  mu : &TraceGenLoggerObject,
                                  new_action : &TraceAction) -> TraceGenLoggerObject {
        let mut new_mu = mu.mu.clone();
        let coloc_id = self.get_lf_coloc_id(new_action.lf_id).unwrap();
        let mut canal = new_mu.get_mut(coloc_id).unwrap();
        canal.push(new_action.clone());
        // ***
