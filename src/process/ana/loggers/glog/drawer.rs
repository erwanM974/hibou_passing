...
use crate::process::ana::param::anakind::{SimulationActionCriterion, SimulationLoopCriterion};
...
impl GraphVizProcessDrawer<AnalysisConfig> for InteractionProcessDrawer {
    ...
    fn make_node_gvitem_as_gvcluster(&self,
        ...
        let (has_simulation,sim_crit_loop,sim_crit_act) = match &param.use_simulation {
            None => {(false,false,false)},
            Some(sim_conf) => {
                (true,sim_conf.loop_crit != SimulationLoopCriterion::None,sim_conf.act_crit != SimulationActionCriterion::None)
            }
        };
